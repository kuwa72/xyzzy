...
// 5b-2: Phase 2 で cp は UTF-16 由来の Unicode code point (u_int32_t、
// surrogate pair は呼び元で 0x10000-0x10FFFF に合成済)。glyph_dbchar は
// 表示幅 2 (wide)、glyph_sbchar は幅 1/0 (narrow / combining) を期待する。
glyph_t *glyph_dbchar (glyph_t *g, u_int32_t cp, int f, int flags);
glyph_t *glyph_sbchar (glyph_t *g, u_int32_t cp, int f, int flags);
