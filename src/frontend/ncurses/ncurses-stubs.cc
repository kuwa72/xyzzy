...
/* **FFI のスタブは消した。** 型の検査と `si:make-c-function' は
   src/core/dll-call.cc、実際に呼ぶ所は src/core/dll-posix.cc にある
...
/* 補完エンジン (class completion と Fdo_completion) は src/core/completion.cc
   ...
   直っている状態が実際に何度も起きていた**: `adjust_prefix' が Char 分しか
   alloca していないのに ucs4_t 単位で書いてスタックを壊す (issue #49)、
   突き合わせが大文字小文字を無視する (issue #111)、ポート作業中の displog が
   13 箇所残る。 */
