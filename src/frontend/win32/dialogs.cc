...
{
  return reinterpret_cast <select_buffer_comparator *> (GetWindowLongPtr (dlg, DWL_USER));
}
...
file_name_dialog_hook (HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  ...
  if (msg == WM_INITDIALOG)
    ...
    {
      ofn = (OFN *)GetWindowLongPtr (hwnd, DWL_USER);
      if (!ofn)
        return 0;
    }
...
IdleDialog::WndProc (HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  ...
  if (msg == WM_INITDIALOG)
    ...
    {
      d = (IdleDialog *)GetWindowLongPtr (hwnd, DWL_USER);
      if (!d)
        return 0;
      if (msg == WM_NCDESTROY)
        {
