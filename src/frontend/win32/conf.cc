...
/* ここに残っているのは**本当に Win32 の話だけ**である。

   * ウィンドウの位置と大きさ — モニタの配置とタスクバーの太さを見て収める
     (WINDOWPLACEMENT / MONITORINFO / GetSystemMetrics)。
   * レジストリからの移行 — 昔の xyzzy はレジストリに設定を置いていた。

   INI の読み書きそのもの (read_conf / write_conf の 17 個の overload) は
   src/core/conf-io.cc へ移した。**Win32 に触っていたのは
   Get/WritePrivateProfileStringW の 2 つだけ**で、それ以外は書式の処理
   だったので、非 Win32 でもそのまま動く (issue #143)。 */
