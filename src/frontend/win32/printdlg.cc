...
subclass_combo::subclass (HWND hwnd_parent, UINT id, WNDPROC wndproc)
{
  ...
  m_owndproc = (WNDPROC)SetWindowLongPtr (m_hwnd, GWL_WNDPROC, (LONG_PTR)wndproc);
}
...
subclass_combo::wndproc (HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  switch (msg)
    ...
    case WM_DESTROY:
      SetWindowLongPtr (hwnd, GWL_WNDPROC, (LONG_PTR)m_owndproc);
      break;
    }
...
print_dialog::notify_spin (NMHDR *nm, const char *unit)
{
  ...
  set_margin_text ((int)GetWindowLongPtr (HWND (SendMessage (nm->hwndFrom,
                                                      UDM_GETBUDDY, 0, 0)),
                                   GWL_ID),
...
print_dialog::set_font_face (int lang) const
{
  ...
  wchar_t buf[LF_FACESIZE + 32];
  wsprintfW (buf, L"%s, %s", wface, point);
...
print_dialog::set_font ()
{
  ...
  lfw.lfCharSet = m_settings.ps_font[lang].charset;
  lfw.lfItalic = m_settings.ps_font[lang].italic;
  ...
  if (ChooseFontW (&cf))
    {
      ...
      m_settings.ps_font[lang].bold = lfw.lfWeight >= 700;
      update_font_size ();
      set_font_face (lang);
      check_proportional_font ();
    }
...
print_dialog::wndproc (HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  ...
  if (msg == WM_INITDIALOG)
    ...
    {
      p = (print_dialog *)GetWindowLongPtr (hwnd, DWL_USER);
      if (!p)
        return 0;
    }
