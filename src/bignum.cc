...
{
  return static_cast <long> (br_len ? (br_len - 1) * BR_SHIFT + ::log2 (double (br_data[br_len - 1])) : 0);
}
...
bignum_rep::howlong () const
{
  ...
  return static_cast <long> ((br_len - 1) * BR_SHIFT + ::log2 (double (x)));
}
...
{
  return (br_len + 1) * BR_SHIFT / (::log2 (double (base)) - 1) + 16;
}
...
ato_bignum_rep (bignum_rep *&br, const Char *p, int pl, int radix)
{
  ...
  const Char *pe = p + pl;
  int width = static_cast <int> (pl * ::log2 (double (radix)) / BR_SHIFT + 1);
  bignum_rep *rep;
