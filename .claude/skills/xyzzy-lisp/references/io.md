...
reference/reference.xml から自動生成。77 エントリ。
...
を使いますが、 with-open-stream を使えば close の手間が省ける上に安全です。
  ...
  :if-exists         : ファイルが存在する場合の動作を指定します。
        ...
        :new-version    ファイルを作成します。
                        既にあれば上書きされます。
        :supersede      :new-version と同じ動作です。
        :rename-and-delete  :new-version と同じ動作です。
        :rename         既存ファイルを FILENAME + "~" に退避してから、
                        新しいファイルを作成します。退避先に既にファイル
                        があれば、それは上書きされます。

        :new-version / :supersede / :rename-and-delete は一時ファイルに
        書いて、close するときに本来の名前へ移します。したがって書いている
        途中でエラーになっても既存ファイルは残ります。（xyzzy はファイルの
        版を持たないので、この3つの動作に違いはありません）
