...
### そのバイナリがリリースかどうかを画面から見分ける

タイトルバーと「ヘルプ」→「について」に出るバージョンは、**タグの上でビルド
したかどうかで変わります。**

```
xyzzy 0.6.0 (Clang/x64)                        <- タグ v0.6.0 の上 = リリース
xyzzy 0.6.0-36-ga34f898a (Clang/x64)           <- v0.6.0 から 36 コミット先
xyzzy 0.6.0-36-ga34f898a-dirty (Clang/x64)     <- さらに未コミットの変更あり
```

出しているのは `git describe --tags --dirty` で、`cmake/git-describe.cmake`
が `src/core/gen/version-describe.h` に書き込みます。**このスクリプトは
ビルドごとに走ります**: コミットは CMakeLists.txt よりずっと頻繁に動くので、
configure 時に captured した値では古くなり、**古いハッシュは無いより悪い**
(走っていないビルドの名前を名乗る) からです。内容が変わったときだけ書き直すので、
コミットが進んでいないビルドで version.cc が再コンパイルされることはありません。

`-dirty` は「未コミットの変更がある」印です。これはどのファイルにも記録されない
状態なので、`-dirty` の付き外れだけは 1 ビルド遅れることがあります。ハッシュ
(リリースと手元のビルドを見分ける部分) は遅れません。
