...
- **追記1 (同じ commit の次の CI run で発覚)**: メモリ不足クラッシュは直ったが、代わりに MSVC x86 で `project-tests.l` が `ファイル入出力エラーです` で失敗するようになった (前回はメモリ不足で他が見えていなかっただけで、これも元からあった可能性がある)。ビルド直後で `_build`/`build` 配下 (除外対象だが走査自体は通過点として触れる) や git チェックアウト直後のファイルに対し、実 Windows 環境固有の一時的なファイルロック/アクセスエラー (Windows Defender のリアルタイムスキャン等) を `file-directory-p` が踏んだものと推測される。Wine では再現しない。`project-ignored-directory-p` の中身を `ignore-errors` で包み、1エントリの stat 失敗が走査全体を巻き込まない (=無視すべきディレクトリではない扱いにしてそのエントリは結果に残す) ようにした。**教訓その2**: `directory :recursive t` で数千エントリを触る `:test` コールバックは、途中の1エントリで環境依存のI/Oエラーが起きても全体を巻き込まないよう `ignore-errors` で守っておくのが安全。
- **追記2 (`ignore-errors` を入れた次の CI run で発覚)**: それでも MSVC x86 が再び `メモリ不足です` でクラッシュした。しかも「ファイル入出力エラー」と「メモリ不足」が commit ごとに入れ替わって出ており、これは決定的なバグ再現ではなく、その CI ランナーのメモリ/タイミング状況次第でどちらの壊れ方をするかが揺れているだけだと判断した。**本当の原因は「`project-tests.l` がテスト対象としてリポジトリ全体 (2000件超のファイル) を直接使っていたこと」自体**: `project-list-files` の再帰・除外ロジックが正しく動くかどうかを検証するのに、リポジトリ全体という巨大かつ将来も肥大化し続ける対象を使う必要は無い。`unittest/data/fname-nonascii/` (filename-nonascii-tests.l) と同じ要領で、専用の小さなフィクスチャディレクトリ (`unittest/data/project-fixture/`, ファイル数件+除外対象ディレクトリ1つ) を作り、そこだけを対象にするよう `test-project-list-files` を書き換えた。**教訓その3**: ファイルシステム走査系の機能をテストするとき、「テスト実行中のリポジトリそのもの」を対象にすると、リポジトリが育つにつれてテストが (特にリソース制約の厳しい環境で) 壊れやすくなる。専用のフィクスチャを使う。
- **追記3 (フィクスチャ作成中に踏んだ別のバグ)**: フィクスチャのルートディレクトリを `(merge-pathnames "unittest/data/project-fixture/" (si:system-root))` で作ったところ、返ってきた値の末尾の `/` が落ちていた (`"Z:/work/unittest/data/project-fixture"` のように)。**`merge-pathnames` は、第1引数がそれ自体 `/` を含む複数階層のパスで末尾も `/` の場合、末尾の `/` を落とすことがある**、という上記トラブルシュート4 (`directory-namestring`) とは別口の merge-pathnames 自体の癖。単純な `(concat root "sub/")` のような文字列連結に置き換えて回避した。**同じ癖を踏んでいる箇所を `grep -n 'merge-pathnames "[^"]*/[^"]*/"'` で探したところ、`lisp/backup.l` の `*backup-directory*` の既定値設定がまさにこれで、末尾 `/` 無しの値が設定されてしまっていた** (ドキュメント上のこの変数の使用例はどれも末尾 `/` 付き)。こちらも `concat` に置き換え、`unittest/defaults-tests.l` に末尾 `/` を確認する回帰テストを追加した。**教訓その4**: `merge-pathnames` の第1引数に「ファイル名」ではなく「複数階層のディレクトリパス」を渡すのは避ける。ディレクトリを組み立てるときは `concat` で確実に文字列連結するか、既存のパスに対して1階層ずつ `merge-pathnames` する。同じパターン (`merge-pathnames "X/Y/"...`) は `unittest/filename-nonascii-tests.l` にも2箇所あるが、そちらは今回のバグの影響範囲外 (現に green) なので未対応のまま残している。
- **追記4 (フィクスチャに縮小した次の CI run でも、なお同じ箇所で `メモリ不足です`)**: `test-project-list-files` の対象を数件のフィクスチャに絞ってもなお MSVC x86 が同じ箇所でクラッシュした。ここで気付いた重要な事実: **`gh run list --workflow=build.yml` でこのブランチの MSVC Build 履歴を遡ると、テストマスキングバグ (上記トラブルシュート3) を直す前の commit `650fe96` では MSVC Build が `success` していた。** つまり `leader-tests.l`/`project-tests.l` がテストマスキングのせいで一度も実行されないまま「成功」していただけで、**MSVC x86 上で `project-tests.l` (project.l のディレクトリ走査を伴うテスト) が実際に一度でも通ったことがあるという記録はこの PR には存在しない**。フィクスチャを数件に縮小してもなお同じ箇所で落ちることから、原因は「project-list-files が扱うデータ量」ではなく、**32bit (x86) ビルドが WOW64 上でアドレス空間 2GB に制限されている**ことだと判断した — `CMakeLists.txt` を見ると `link.exe` に `/LARGEADDRESSAWARE` が指定されておらず (対して mingw/lld 側の `NOT MSVC` 分岐は既定でこれを付ける)、682件のテスト・20以上のテストファイル・このPRで追加された新規ライブラリ (`leader.l`/`project.l`/`git.l`/`terminal.l`/`fuzzy-complete.l`) を1プロセスに読み込み続けた末に、ちょうど project-tests.l のあたりでアドレス空間の天井 (2GB) に達していたと推測される。**`llvm-mingw i686` (Wine, これも32bit) は同じテストで一度も落ちていない** — mingw/lld は大アドレス空間対応を既定で付けるのに対し MSVC の `link.exe` は付けない、という違いがちょうど符合する。`xyzzy`/`xyzzy-batch`/`xyzzy-cli` の MSVC リンクオプションに `/LARGEADDRESSAWARE` を追加し (使用可能アドレス空間を 2GB→4GB に拡大、64bit ビルドでは既定なので無害)、CI 待ち。**教訓その5**: 「小さくしたのに同じ場所で同じ壊れ方をする」ときは、対象データのサイズを疑う前に、そのビルド設定・プロセス自体の制約 (アドレス空間、スタックサイズ等) を疑う。特に 32bit ビルドで、他のツールチェイン (今回は mingw) では再現しないなら、リンカ設定の差を比較する。
- **追記5 (`/LARGEADDRESSAWARE` は外れ、本当の原因が判明)**: `/LARGEADDRESSAWARE` を追加した次の CI run で、**MSVC x64/ARM64 (64bit、アドレス空間の制約が実質無い) も同じ場所で failure/timeout した**。これで「32bit のアドレス空間」説は完全に否定された (**教訓その5は誤りだったということではなく、"同じ壊れ方が続くなら前提を疑う" という姿勢自体は正しかった** — 次に疑うべき前提が「32bit特有」ではなく「このコードパス自体」だったというだけ)。`/LARGEADDRESSAWARE` 自体は 32bit ビルドの一般的なプラクティスとして残しているが、今回の直接の修正ではない。
  この時点で `run-project-unit-tests` の各ステップに `format`+`finish-output` のブレッドクラムを追加し (`unittest/project-tests.l`)、さらに `project-find-root-directory`/`project-current-root` 内部にもイテレーション番号・マーカーチェック単位のブレッドクラムを追加 (`lisp/project.l`) して CI に投げたところ、**ローカル (Wine) で `project-find-root-directory` 自体の内部動作を精査でき、真因を特定できた**: `(truename start-dir)` は末尾の `/` を落とす (`"/work/"` → `"Z:/work"`)。これをそのまま `directory-namestring` に通すと、`start-dir` がディレクトリであっても最後のディレクトリ名をファイル名と誤認して1階層上から探索を始めてしまう。今回のフィクスチャ (`unittest/data/project-fixture/`, その1階層上の `unittest/data/` にはマーカーが無い) では、誤った開始点から**さらに上へ登り続けてリポジトリ自身のルート (`CMakeLists.txt` がある) に辿り着き、フィクスチャのルートではなくリポジトリのルートを返してしまう**という、目に見えにくい形で発現していた。
  **`file-directory-p` で分岐し、ディレクトリなら末尾 `/` を補うだけにする (`directory-namestring` に通さない) 修正を `project-find-root-directory` に入れたところ、mingw(Wine) 3種・MSVC x86/x64/ARM64 の6ジョブ全てが green になり、所要時間も (それまでの20分超から) 8〜9分の正常な範囲に戻った。** つまり、この trailing-slash バグそのものが MSVC 環境でのハング/メモリ不足/タイムアウトの直接原因だった (Wine 環境ではこの誤りが単に「間違った答えを返す」だけで済んでいたのに対し、MSVC 環境では `truename`/`file-exist-p` の何らかの組み合わせが極端に遅い/リークする経路に入っていたと考えられる — 具体的な native 側のメカニズムまでは未解明だが、再現条件と修正の効果から因果関係は確度が高い)。回帰テスト `test-project-root-detection-exact-directory` を追加済み。
  **教訓その6**: ローカルで再現しない CI 専用の不具合を追うとき、「疑わしい関数の中に直接ブレッドクラムを埋め込んで CI に投げる」のは一見コストが高く見えるが、外側から間接的に絞り込むより早く核心に辿り着けることがある。特に今回のように、バグが「間違った答えを返しつつも見た目には動いているように見える」(フォールバックや偶然の一致で隠れる) タイプだと、通常のテストでは決して発覚しない。
...
3. ~~PR のマージ・レビュー対応（CI チェック通過確認）~~ **完了**。commit `0e9e27d` で `mingw` (Wine) 3種・`Build` (MSVC x86/x64/ARM64) 3種の計6ジョブ全てが green (各8〜9分)。道のりの記録: `mingw` が green だからといって `Build` (MSVC) も通っているとは限らない — この PR では `546b158` 時点で `mingw` は green だったが `Build` は未実行/キャンセルのままで、その後 `Build` が実際に走ってから上記トラブルシュート7の一連の不具合が次々に見つかった。両方の required check の green を必ず確認すること。
