...
- **同じシンボルのまとめ改名 (`lisp/iedit.l`)**:
  - `iedit` (`Leader c r`): カーソル位置のシンボル (または選択範囲) と同じものをバッファ全体でまとめて書き換える。打った文字がその場で全部の箇所に入り、`BS` で消え、`RET` で確定、`C-g` で元に戻る。`foobar` のようにシンボルの途中に埋まっているものは巻き込まない。
  - **打鍵を自分で読むモーダルな作りにしてある。** `*self-insert-hook*` は関数 1 つしか持てず、既に autopair が入っているため。代わりに名前の途中へカーソルを移すことはできない (末尾への追加と `BS` だけ)。
  - **位置はマーカーではなく置換の差分から計算する** (置換はどこでも同じ長さの入れ替えなので、k 番目は差分 × k だけずれる)。**書く前にその箇所が本当に想定の文字列かを確かめ、違えば何も書かない**: 計算がずれているということなので、そのまま書くとバッファが壊れる。
