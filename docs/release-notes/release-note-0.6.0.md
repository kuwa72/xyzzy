...
**きっかけはターミナルの不具合である。** Shell バッファを開いたまま放置すると、
戻ってきたときにキー入力が画面に反映されなくなる。synchronized output の
タイムアウトを壁時計で計っていたためで、monotonic clock に切り替えて直した。
ターミナルを常用していると効き方が大きいので、ここで一度出す。

**中身の大半は #16 のロードマップである。** C++17 への引き上げとレガシーな
自前実装の整理 (Phase 1)、Common Lisp 仕様との差異の修正 (Phase 2)、
Lisp コンパイラの最適化 (Phase 3)、Core と Frontend の境界整理 (Phase 4) を
順に進めた。**この手の作業の価値は、実際に出てきたバグの数で測るのが正直だと
思う。** 型を揃える・テストが無いから足す・偽装をやめる、といった地味な作業の
途中で、10 件を超える実バグが出てきた。GC が多値バッファの未初期化領域まで
ルートとして走査していて実 Windows でアクセス違反を起こしていたもの、
ncurses 側のヒープバッファオーバーフロー、`subtypep` の `and`/`or` の
ロジックが左右逆だったもの、`mismatch` の `:from-end` が完全に無視されて
いたものなどである。

**Linux ネイティブ版が「起動して編集して保存できる」ところまで来た。** これまで
`src/frontend/ncurses` と `src/frontend/cli` はどの CI ジョブでもコンパイルすら
されておらず、実際にビルドが通らず・通しても起動しない状態まで腐っていた。
gcc でのネイティブビルドを CI に載せ、そこで見つかった故障を直し、POSIX の
ファイルシステム実装をフロントエンドからコアへ移した。まだ FFI・tree-sitter
ハイライト・設定の永続化が無く、Lisp テストスイートも Linux では走らないので、
Windows 版と同じ土俵に乗ったわけではない (#49, #50)。
...
大量の bignum を生成するループや深い末尾再帰で実 Windows のみクラッシュする、
という報告 (#43) を追ったもの。TCO の追加で「深い末尾再帰を実用的に回せる」
ようになったことで顕在化したが、バグ自体は以前から存在していた。
...
最適化の主戦場をバイトコンパイラ (`lisp/compile.l`) に置く方針で、VM の
呼び出し規約には手を付けていない。全アーキで同じように効く。
...
いずれも「実装を読んで仕様と突き合わせ、テストが無いところにテストを足す」
という進め方で、その過程で実バグが出てきたものである。
...
どちらもテストを書く側が踏む落とし穴で、コードの変更ではないが、次に同じ
ところで時間を溶かさないために残しておく。
...
----------
  ...
  * **Linux 版で Lisp テストスイートが走らない。** `misc/run-tests-batch.l` を
    流すと、一切出力を出さないまま CPU 100% でメモリを食い続ける
    (25 分で 12GB まで確認。原因未調査、#49)。このため Linux 側の CI が見て
    いるのは `tools/linux-smoke.sh` (起動 → ライブラリのロード → 式の評価 →
    ファイル操作) だけで、機能的な正しさは保証されていない。Windows 版は
    従来どおり既知失敗リストで gate してある。
  * **Linux 版に欠けている機能。** DLL のロードが無いため FFI・tree-sitter
    ベースのハイライト・プラグイン DLL は動かない。C++ 層の設定 (ini /
    レジストリ) は永続化されない。TLS (HTTPS) とクリップボードの OS 連携も
    無い。一覧と優先順は #50 にある。
  * **`search` が実装されていない。** ANSI CL のシーケンス関数のうちこれだけ
    欠けている (Phase 2 のキーワード引数監査の対象外だった)。
  * **`with-standard-io-syntax` が実装されていない。** ANSI CL の標準マクロで、
    印字/読込の動的変数群を標準値に束縛する。C++ コア側に `*print-case*`/
    `*print-gensym*`/`*print-array*`/`*read-base*` が存在しないため、
    対応する場合は既存の変数のみを束縛する縮小スコープでの実装になる。
