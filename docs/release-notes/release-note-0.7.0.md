...
Lisp テストスイートは走るようになり (走っていなかった -> 1300 件)、既知失敗は
158 件から 8 件に減った。残る 8 件は**全部「POSIX に対応物が無い」側**である:
stdcall のコールバック、`GetLastError` (2 件)、待機オブジェクト (2 件)、
OLE (2 件)、`abbreviate-display-string`。

**端末版が「動く」から「速い」に変わったのもこの回である。** ダンプイメージを
既定で使うようにして、**対話起動が 832ms から 178ms になった**。ソケットと
名前解決は POSIX で使えるようになり、**待っている間に `C-g` で抜けられる**
(それまでは待ちに入ると打鍵が届かなかった)。非同期のプロセスは**出力が
どこにも出ていなかった**のを直し、その上で flymake の自動実行が待たなく
なった。端末で貼り付けるとインデントが積み上がる問題は bracketed paste で
解決した。最後に、**カーソルが点と別の所に出ていた**のを直している (画面は
正しいのでカーソルだけがずれる、という見え方をしていた)。

**Windows 版と同じ土俵に乗ったわけではない。** FFI は動くようになったが
(#133)、`misc/known-failures/linux.txt` の 8 件が残っており、Lisp スイートの
件数も Windows 版のほうが多い (1300 対 1314)。足りない所の一覧は #50 である。
...
------------------------------
  ...
  * **保存したカーソルを画面ごとに持つようにした** (PR #29)。DECSC (`ESC 7`) /
    DECRC (`ESC 8`) と `CSI ? 1049 h/l` が**同じ 1 つの保存位置**を使って
    いたので、alternate 画面の中で DECSC した TUI が primary 側の保存位置を
    壊し、戻ったときに違う場所へカーソルが行っていた。primary と alternate で
    別に持つ形にした。

    **`1049` に専用の保存位置を作ってはいない。** 元の PR はそうしていたが、
    xterm の ctlseqs は `1049h` を「DECSC と同じ所へ保存」、`1049l` を
    「DECRC と同じ所から復元」と定義している。つまり
    `DECSC` -> `1049h` -> `1049l` -> `DECRC` の順で来たら、**最後の DECRC は
    1049h が保存した位置へ戻るのが正しい。** 専用の位置を作るとここが変わる
    ので、その境界をテストで押さえた
    (`terminal-mode-1049-shares-the-decsc-slot`)。

    DECSC / DECRC が**色 (SGR) と origin mode も保存する**ようにした
    (xterm は保存する。位置だけ戻して色が戻らないと、復帰後の出力の色が
    変わる)。`unittest/terminal-tests.l` に 4 件。
  * **ConPTY の reader スレッドと描画を排他した** (PR #28)。Windows では
    ConPTY の出力を読むのが別スレッドで、そこから `Terminal::feed ()` を
    呼んでいる。GUI スレッドは同じ格子を読んで描き、リサイズでは作り直すので、
    **ロックが 1 つも無かった。**

    **これは「直った」と書けない類の修正である。** データ競合は CI では
    測れない (Wine にも ConPty はあるので経路は走るが、競合したことを
    検出する手立てが無い) ので、言えるのは「覆った範囲」だけ:
    `feed` / 描画 / リサイズは覆った。**GUI 側の残り** (scrollback の
    スクロール、スクロールバーの寸法、選択範囲のコピー) は覆っていないので
    #264 に書いた。POSIX 側は reader スレッドを持たない (`poll` で回す) ので
    この話は Windows だけである。
...
--------------------------
  ...
  * **リリース本文を GitHub の上限に収める道具を足した**
    (`tools/release-body.py`)。リリースノートをそのまま本文にしていたが、
    **GitHub のリリース本文は 125,000 文字まで**で、この回のノート
    (183,333 文字) は入らない。**3 アーキのビルドが全部終わった後に**
    HTTP 422 "body is too long" で落ち、タグは push 済みでリリースは無い、
    という形になった (この回で実際に踏んだ)。

    入るときはノートをそのまま渡す (0.6.0 のノートで 1 文字も変わらないことを
    確かめてある)。入らないときは**「イントロ + 節と件数 + 既知の問題 +
    全文へのリンク」**にする。**全文を捨てるのではなく、置き場所をリポジトリに
    移すだけ**で、タグに紐付いた URL を本文の最後に置く。

    **ビルドの前に見るようにした。** 本文が作れるかは 10 秒で分かるのに、
    その後ろのビルドは 25 分かかる。`Check the tag` の隣に同じスクリプトを
    走らせる step を置いた。イントロだけで上限を超える場合はスクリプトが
    非 0 で終わる (**API に蹴られる文字列を出さない**)。

  * **表示用のバージョンが、素のバージョンで始まるようにした。** タイトルバーと
    「について」に出る文字列は `git describe` から作っていたが、**タグの名前は
    「前の」リリースを指す**ので、`CMakeLists.txt` を上げてからタグを打つまでの
    間、**0.7.0 のビルドが自分を `0.6.0-144-gfc2e845e` と名乗っていた。**
    describe の後ろ半分 (`-144-gfc2e845e-dirty`) だけを使い、頭には
    `PROGRAM_VERSION` を置く形にした。

    **CI では絶対に見えない不具合である。** actions の checkout はタグを
    持たないので `git describe` が失敗し、素のバージョンに落ちる。
    `version-display-string-starts-with-the-plain-version` は元からあって
    「リリースビルドでだけ落ちるテストを書くな」と docstring に書いてあったが、
    落ちたのはその裏返し (bump とタグの間) で、**手元でだけ落ちた。**
...
----------
  ...
  * **端末版 (Linux / POSIX) に残っている 8 件。** `misc/known-failures/linux.txt`
    に載っているもので、**全部「POSIX に対応物が無い」側**である: stdcall の
    コールバック、`GetLastError` (2 件)、待機オブジェクト (2 件)、OLE (2 件)、
    `abbreviate-display-string`。一覧と優先順は #50 にある。
  * **端末版に TLS (HTTPS) が無い。** `src/core/sockssl.cc` は全体が
    `#ifdef _WIN32` で、中身は SChannel である。平文のソケットと名前解決は
    この回から POSIX でも動く (`C-g` で待ちを抜けられる)。
  * **Windows の `make-process` の経路が変わったが、実機で測っていない**
    (#253)。`:terminal t` を渡したときだけ ConPty を使い、渡さないときは生の
    パイプになった。**CI の Wine には ConPty があるので両方の経路が走って
    いる**が、Windows 10 1809 以降の実機で `M-x shell` を端末として使う所は
    確かめていない。
  * **`search` が実装されていない。** ANSI CL のシーケンス関数のうちこれだけ
    欠けている。
  * **`with-standard-io-syntax` が実装されていない。** 対応する場合、C++ コア側
    に `*print-case*` / `*print-gensym*` / `*print-array*` / `*read-base*` が
    無いので、既存の変数だけを束縛する縮小スコープの実装になる。
  * **CI で散発的に落ちるものが 2 つある。** ARM64 の生成ステップが Wine の
    `ShellExecuteEx` で落ちるもの (#247) と、`user-config-path` の起動オプション
    のテスト (#253)。どちらも再実行で通り、原因はまだ 1 回しか観測していない。
    **数えるための issue として置いてある。**
