...
----
  ...
  * **端末のポップアップ (`popup-list` / `popup-string`) に確認を 2 件足した。**
    ...
    **飾りではなく既定の経路である。** `lisp/startup.l` が端末で
    ...
    見るのは 3 つ: 枠が出ること / **枠の上下の桁が揃っていること** /
    **選んだ文字列がコールバックへ渡ること。** 2 つ目は幅を「文字数」で
    数えると日本語の項目で崩れるところで、罫線は ASCII なので桁数をそのまま
    比べられる。3 つ目が要るのは、**描くだけ描けて選択が効かない形**が
    あるため。

    項目は 20 桁の日本語にしてある。**`inner_width` に 13 桁の下限がある**
    ので、5 文字 (10 桁) では文字数で数えても下限に吸われて差が出ない。
  ...
  * **`--batch` の標準出力が CP932 + CRLF だったのを UTF-8 + LF にした**
    ...
    ```
    $ xyzzy --batch -q -e '(car 1)'
    変更前: \x95\x73\x90\xb3...   (CP932 の「不正なデータ型です」)
    変更後: 不正なデータ型です
    ```
