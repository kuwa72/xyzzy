...
----
  ...
  * **ミニバッファ補完の TAB 連打で候補を順に切り替え**:
    `find-file` を含むミニバッファ補完で、TAB を繰り返し押すと候補が順に挿入されるようになりました。
    1 回目は従来通り共通接頭辞までの補完と一覧表示を行い、2 回目以降の TAB で候補をひとつずつ切り替えます。
    編集するとサイクルは解除され、`RET` での確定は従来通りです。
    `*minibuffer-completion-cycle*` を nil にすると従来の動作 (一覧表示どまり) に戻ります。
  * **半角スペース等の表示記号が出ない問題の修正**:
    改行・タブ・半角/全角スペース等の bitmap 表示記号が、起動直後から表示されず、スクロールしても直らないことがある問題を修正しました。
    記号画像 (glyph atlas) の生成がフォント変更時にしか行われておらず、起動直後は空のまま全 blit が空振りしていました。
    起動時のフォント生成でも atlas を作るように戻しました。Windows 上での検証手順は `tools/verify-half-width-space.ps1`。
