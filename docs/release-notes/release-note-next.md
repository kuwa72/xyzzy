...
----
  ...
  * **`src/core/Window.h` から mode_line_painter 系 3 クラスを `src/frontend/win32/` 側へ移した** (issue #294)。`mode_line_painter` / `mode_line_percent_painter` / `mode_line_point_painter` とそれらの状態を `Window` から分離し、新規 `modeline-painter.h` に集めた。`Window` には `void *w_mode_line_state` を置き、Win32 側だけで実体を確保・解放する。Win32 GUI 依存を core から frontend へ出す #277 の分割 1/3。
  * **`src/core/Window.h` の `w_hwnd` / `w_hwnd_ml` を opaque な `void *` にし、HWND 値はフロントエンド側で管理するようにした** (issue #295)。`Window` には `void *` として保持し、Win32 フロントエンドが `hwnd()` / `hwnd_ml()` アクセサを通じて `HWND` として読む。ウィンドウオブジェクトと Win32 ハンドルが 1 対 1 かつ生成・破棄タイミングが一致しているため、別テーブルを持つよりフィールドを opaque 化する方が単純である。
  * **`src/core/Window.h` から Win32 HDC を直接扱う描画 API を取り除き、描画は `Painter &` 経由に統一した** (issue #296)。残っていた `paint_window` / `paint_region` / `paint_mode_line` / `paint_terminal` / `paint_background` / `paint_ruler` などの `HDC` 版を削除し、Win32 フロントエンドの呼び出し側で `Win32Painter` を明示的に構築する。caret 更新や frame ウィンドウサイズ変更は frontend 自由関数 (`update_caret` / `frame_window_resize` / `frame_window_setcursor`) として `src/frontend/win32/` 側に置き、core から Win32 GUI 依存をさらに減らした。これで #277 の最後の段階が完了する。
  * **Win32 GUI 分離の不変条件をテストで固定した** (issues #281, #294, #295, #296)。実行時条件は `unittest/win32-separation-tests.l` (POSIX の `format-drive` unsupported 化、リフレッシュ経路の無事動作) で、ソース構造条件 (`src/core` への HDC / HWND / `mode_line_painter` 漏れなど) は `tools/check-win32-separation.py` で `tools/run-tests.sh` から検査する。
  * **`src/core/ed.h` から core 側で使われていない HWND 5 件を `src/frontend/win32/` 側のグローバル変数に出した** (issue #297)。`StatusWindow::sw_hwnd` / `StatusWindow::set` / `Frame::hwnd` / `Frame::{has_caret, has_caret_last}` / `Application::hwnd_sw` を削除し、`src/frontend/win32/ed-hwnd.h` 経由で `g_status_window_hwnd` / `g_active_frame_hwnd` / `g_active_frame_has_caret` / `g_active_frame_has_caret_last` / `g_app_hwnd_sw` として frontend 側で管理する。
