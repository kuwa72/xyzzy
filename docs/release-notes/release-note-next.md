...
----
  ...
  * **端末版のカーソルが点の居る所に出るようにした** (issue #255)。
    3 行目に打ち込んでいるのにカーソルは 2 行目の左端に居る、という状態
    だった。**字は点に入るので画面そのものは正しく、ずれているのは
    カーソルだけ。** どちらが正しいのかも画面からは分からない。

    原因は「同じ位置を 2 箇所で別々に計算していた」ことで、4 つ出てきた:

      * **ルーラの行を数えていない。** カーソルを置く側は `w_rect.top + cy`、
        描く側は `w_rect.top + ルーラの行数 + cy`。**ルーラを既定で出すように
        した issue #173 の時点から**ずれていた。ポップアップの位置決め
        2 箇所も同じ計算をしていた。**足す場所を 1 箇所
        (`point_to_screen`) にまとめて、呼ぶ側で足せないようにした。**
      * **`w_flags` を見ていた。** `w_flags` はそのウィンドウで明示的に
        上書きした分しか持たず、既定 (`w_default_flags`) とバッファ毎の分を
        混ぜるのは `flags ()` の側。行番号は既定で出るので、`w_flags` で
        見ると**出しているのに 0 桁として数え**、カーソルが
        `LINENUM_COLUMNS + 1` = 7 桁左に来る。
      * **横スクロールの幅も同じ間違いだった。** 幅が 7 桁広いので、**点が
        右端の外へ出てから**流れる。外へ出た `move` は失敗するので、
        カーソルは前に描いた所 — 画面の左下 — に残る。120 桁の行を 100 桁の
        画面で測ると `cursor 29,0` になる。
      * **マウスでクリックした所と点の入る所も同じ理由で食い違っていた**
        (画面の位置から行と桁を求める逆写像が同じ数え方をしていない)。
        1 行目の 1 桁目を押すと**2 行目の 2 桁目**に入っていた -- ルーラの行と
        左余白 1 桁の両方を数え損ねている。
      * **ルーラを reframe より前に描いていた。** 目盛は `w_top_column` から
        作るので、横スクロールを決める前に描くと**1 回前の位置の目盛**が出る。
        テキストが 32 桁目から出ているのにルーラは 0 から始まり、次の打鍵で
        ようやく揃う (打鍵が止まると再描画も止まるので、揃わないまま残る)。

    **道具の側にも穴があった。** `tools/pty-drive.py` は docstring に
    「カーソルがどこに居るか」を見る道具だと書いてあるのに、**カーソル位置を
    出していなかった。** 出していないので、この不具合は dump では
    「正しい画面」に見える。`=== cursor 行,桁` を出すようにして、
    `tools/linux-smoke.sh` に 3 件足した (点と同じ所に居ること / クリックした
    所に点が入ること / 横に流れても付いていくこと)。**流れた桁数は焼き付けていない**: カーソルは
    行末の `[EOF` の直前に居ること、ルーラは**目盛の `+` が来ている桁の
    テキストが `0` か `5`** (`+` はバッファの 5 の倍数の桁に付き、測るのに
    使う行は桁番号の下 1 桁を並べたもの) で見ているので、流れる量が変わっても
    成り立つ。
  * **`tools/x build` が、configure していない arch なら自分で configure する
    ようにした。** それまでは cmake の
    `Error: /work/_build/i686 is not a directory` で止まり、**何を打てばよいのか
    がどこにも書いていない。** 実際に困ったのは `tools/deploy-windows.sh` で、
    x86_64 を出し終えた**後**に i686 でこれに当たるので、
    「片方だけ更新された配布ディレクトリ」を残して落ちる。

    **configure を明示的に打つ意味は残してある。** `tools/x configure ARCH ...`
    は常に走る (引数を渡し直したいのはこちら)。自動で呼ぶのは
    `CMakeCache.txt` が無いときだけで、既に configure 済みの build ツリーの
    設定を黙って上書きすることはない。
  * **flymake の自動実行が待たなくなった** (issue #248)。
    `*flymake-idle-delay*` を入れると**入力が止まるたびにチェッカの終わるまで
    エディタが固まっていた** (gcc のような重いものだと体感で分かる)。

    **`M-x flymake-check` は同期のままにしてある。** 戻った時点で印が付いて
    いることを期待される対話コマンドで、`unittest/flymake-tests.l` の 3 件も
    その形で書いてある。**変えると「呼んだのに何も起きない」に見える。**
    待って困るのは自動で走る側だけなので、そこだけ変えた
    (`*flymake-on-save*` と `*flymake-idle-delay*` の 2 つの入口)。

    **出力はフィルタで溜める。** sentinel の時点でバッファが揃っているかに
    頼らない形にした。`:terminal` は渡さない -- 端末エミュレータを作られると
    出力が来ない (issue #250)。

    **走らせてから編集されたら結果を捨てる。** 行が動いた後に印を付けると
    位置が合わない。**ずれた印を付けるくらいなら付けない方がまし**で、
    入力が止まればまた走る (`buffer-modified-count` を launch 時に控えて
    突き合わせる)。

    **重ねて起動しない印を持つ。** 入力が止まるたびにプロセスが積み上がる。
    印はバッファごとで、**`defvar` では宣言していない** -- special な変数は
    バッファローカルになれない (`lisp/startup.l` の `*buffer-package*` の
    注記と同じ理由)。

    テストは 5 件足した。**遅いコマンドが要るものは unix だけ** (cmd.exe に
    `sleep` が無い)。**待ち方は期限付き**で、固定の `sleep-for` では書かない
    (子プロセスの往復を 1 つの枠に押し込むと機械が混んでいると落ちる)。

    負の確認 3 つ。**それぞれ狙った 1 件だけが落ちる:**

      * アイドルの配線を同期へ戻す -> `flymake-idle-check-does-not-block`
      * stale の判定を外す -> `flymake-check-async-drops-a-stale-result`
      * 重複起動の印を外す -> `flymake-check-async-does-not-stack`

    **1 つめのために配線のテストを足した。** 他の 4 件は
    `flymake-check-async` を直に呼ぶので、**配線を戻しても落ちなかった。**

  * **非同期のプロセスの出力がどこにも出ていなかったのを直した**
    (issue #250)。`make-process` に `:terminal` を足して、**端末エミュレータは
    頼まれたときだけ作る**ようにした。

    ```
    (execute-subprocess "echo hello")     ; M-x のメニューにもある
    変更前: *Command Output* が空。画面にも何も出ない
    変更後: hello
    ```

    **`set-process-filter` も一度も呼ばれていなかった。** フィルタを呼ぶのは
    `Process::insert_output` の中で、そこへ一度も来ていなかった。

    **出力は消えていたのではなく、端末エミュレータへ入っていた。**
    `Process::create` が必ず `Terminal` を作るので `poll_output` は常に
    そちらへ食わせ、**バッファへ挿入する `insert_output` に来ない。**
    そして `terminated ()` がプロセスを `Vprocess_list` から外すので、
    エミュレータを探す `buffer_terminal` が 0 を返すようになる --
    **子が終わった瞬間にエミュレータごと辿れなくなる。** `echo` のような
    短い命令では**一度も見えない。**

    端末が要るのは `M-x shell` (eshell) のように画面を持つ子だけである。
    `execute-subprocess` や flymake のようにテキストが欲しい側は要らない。
    **頼む側が `:terminal t` を渡す**形にした (渡すのは `lisp/shell.l` の
    eshell だけ)。

    **Win32 も同じ形にした。** ConPty が使える環境では常に ConPty で、
    そちらも端末エミュレータへ食わせるので同じ性質だった。`:terminal` が
    無ければ ConPty より前からある生パイプの経路 (`NormalProcess`) を使う --
    **そちらはバッファへ挿入し、フィルタも呼ぶ。**

    ついでに出た: **バッファへ入れる経路では pty の ONLCR を切る必要がある。**
    pty の行規則が `\n` を `\r\n` にするので、直した直後は
    `"hello\x0d\x0a"` が入っていた。エコーも切る (送った分が返ってきて
    出力に混ざる)。

    `unittest/process-async-tests.l` に 5 件足した。**どれも一度も測られて
    いなかった** -- #50 の「サブプロセスは動く」は `call-process` (同期) で
    確かめたもので、非同期の側は測っていなかった。

    負の確認: `Terminal` を無条件に作る形へ戻す -> **5 件すべてが落ちる。**
    最初は 3 件しか落ちなかった (「CR が無い」と「バッファが空」は**何も
    流れなかったときにも通る**) ので、**中身が来ていることも見る**形に
    直した。

    これで **#248 (flymake を非同期にする) が書けるようになった** --
    出力が取れないので塞がっていた。

  * **端末で貼り付けるとインデントが積み上がったのを直した**
    (issue #241)。bracketed paste (DECSET 2004) を実装した。

    ```
    送った (c-mode)              変更前                    変更後
    int f(int x) {               int f(int x) {            int f(int x) {
      if (x) {                     if (x) {                  if (x) {
        return 1;                      return 1;   <- 6 桁       return 1;   <- 4 桁
      }                            }                         }
    }                            }                         }
    ```

    **貼り付けと打鍵が区別できていなかった。** 端末の貼り付けはテキストを
    そのまま打鍵として送るので、自動インデント・自動ペア・electric が
    1 文字ずつに反応する。`{` の後の RET で c-mode が 2 桁入れ、そこへ
    貼り付け側の 4 桁が乗って 6 桁になっていた。**`lisp-mode` では出ない**
    (RET が自動インデントしない) ので、**モードによって出る。**

    端末に `ESC[?2004h` を送ると、端末が貼り付けを `ESC[200~` …
    `ESC[201~` で囲んで送ってくる。**囲みは `define_key` で ncurses に
    教える** -- 教えないと `ESC[2` までがキーの並びとして食われて、残った
    `00~` がテキストとしてバッファに入る (実測した)。

    **中身は 1 文字ずつ流さない。** 囲みの中を読み切って溜め、キューへは
    「貼り付けが来た」1 つ (`LCHAR_PASTE`) だけを入れる。`dispatch` が
    それを見て `bracketed-paste-function` を走らせ、そのコマンドが
    `si:*take-pasted-text` で受け取って **1 回の `insert`** で入れる。
    フックを通らないことが仕組みとして保証され、undo も 1 回で戻る。

    **`*self-insert-hook*` を見る作りにしなかった**のは、見ないことが目的
    だから -- フックの側で「貼り付け中か」を判定する形だと、判定を忘れた
    分だけ崩れる。

    測り方が自己完結している: **同じテキストを囲んで送ると 4 桁、囲まずに
    送ると 6 桁**になることを 1 つの check で見る。**後者が無いと空回りする**
    (c-mode が自動インデントしない設定になっただけでも前者は通る)。

    ### 途中で 2 回、自分で回帰を作った

    **`kind` を `&` で見分けてはいけない** ことを 2 通りの形で踏んだ。
    `LCHAR_MENU` は kind 4、新しい `LCHAR_PASTE` は kind 5 で、**bit 2 を
    共有する。**

      * `cc & LCHAR_PASTE` と書いたら**メニューから選んだコマンドが走らなく
        なった** (`LCHAR_MENU` にも当たるので、menu が paste の枝へ落ちた)。
        smoke の「メニューの実行」が落ちて分かった -- **今日足した check が
        今日の回帰を捕まえた**
      * union の判定 `c & (LCHAR_MENU | LCHAR_PASTE)` は **`7 << 21` =
        kind mask 全体**になるので、**function key (kind 1) にも当たり**、
        `terminal-key-*` が 8 件まとめて落ちた

    `LCHAR_KIND (cc) == ...` で比べる形に直し、union には
    `lchar_event_p` を用意した。**両方の罠を `chtype.h` に書いた。**

    もう 1 つ: **`si:*take-pasted-text` を `lisp/builtin.l` に宣言し忘れた。**
    `test-defun-builtin-1` が「builtin.l で定義されていない builtin 関数が
    存在しないこと」を見ていて落ちた。**規約がテストで守られている。**

    Windows では **`xyzzy-cli.exe` だけがリンクに失敗した。** スタブを
    `cli-stubs.cc` の `#ifndef _WIN32` の中に置いたためで、あれは 4 つある
    link 先の 1 つである。

    負の確認: `define_key` を消す -> 囲みが届かず、この check が落ちる。

  * **遅い DNS でエディタが数秒固まり、C-g でも戻れなかったのを直した**
    (issue #223 の段取り 2)。名前解決を **worker スレッドで引いて、main は
    100ms 刻みで C-g を見ながら待つ**形にした。

    `resolver.h` の非 Win32 側は `gethostbyname` を直に呼ぶだけだった。
    **引けてはいた** (issue #223 に「名前解決が無い」と書いたのは誤りで、
    同期版は前からあった) が、**あの呼びには `poll_quit_char` を挟む隙が
    無い。**

    **なぜスレッドか。** fd を持たないブロッキング呼び出しなので、既にある
    「fd を主 `select` に入れる」形 (サブプロセス) には乗らない。子プロセスで
    引く案は、Lisp ヒープごと `fork` するのが名前解決 1 回には重いうえ、
    **tree-sitter が既にスレッドを使っているので multi-threaded な
    プロセスからの `fork`** になる。worker が触るのは自分の箱だけで Lisp を
    触らないので、`gc_mark_in_stack` の制約 (走査するスタックは 1 本) に
    抵触しない。

    `gethostbyname` は静的な領域を返すのでスレッドから呼べない。
    **`getaddrinfo` / `getnameinfo` に替えた** (呼ぶ側の領域に書く)。

    **箱の持ち主が 2 人いる。** main が諦めた (C-g / 期限切れ) 後も worker は
    まだ書いているので、**参照数を 2 で始めて後に手を離した方が解放する。**
    読む順序も決まっていて、worker は結果を書いてから最後に `done` を立て、
    main は `done` を見てから読む (印は SEQ_CST)。諦めたときは読まない。

    **結果の寿命は Win32 と同じ約束**にした: 返す `hostent` は resolver の
    メンバを指し、**次の lookup まで有効。** `sockinet::saddr::hostname ()` が
    `h_name` をそのまま返すので、この約束が要る。

    **`EAI_*` の category は足していない。** `h_errno` の 4 値へ写して、
    PR #228 で作って測ってある `DNS_ERROR` の経路をそのまま使う。写して
    落ちる情報が実務で問題になったら、そのとき category を足す。

    測り方はソケットの側 (#242) と同じで**時間を測らない**。C-g の後の eval が
    描かれること、かつ**中断が `quit` として届くこと** (解決が自分で戻った
    のではないこと) の 2 つ。

    **この 1 件は CI では SKIPPED になる。** 待ちを起こすには応答しない
    ネームサーバを指す必要があり、`/etc/resolv.conf` を書き換えるのは
    **使い捨てのコンテナの中でしかやってはいけない** (CI の runner や開発機の
    DNS を壊す)。`tools/x smoke` を回すところでは実際に測れるので置いてある。
    **黙って飛ばさず SKIPPED と言う** -- 「測っていない」と「通った」を混ぜない
    ため。

    負の確認: resolver の待ちから `poll_quit_char` を消す -> **この check だけ**が
    落ちる (ソケットの側は別の呼び出し場所なので通ったまま)。

  * **worker スレッドの土台を tree-sitter の中から出した**
    (`src/core/worker-thread.h`、issue #223 の下ごしらえ)。**挙動は変わって
    いない。**

    起こす / 期限付きで待つ / 手放す / CPU を譲る の 4 操作と、印を書く
    atomic 1 つ。`src/core/ts.cc` の中に閉じていたものを、**名前解決が
    2 つめの使い手になるので出した。** 呼び名を `ts_*` から `worker_*` へ
    変えただけで中身は同じ。

    **3 つめの写しを作らないために出した。** このリポジトリは「同じことを
    するコードが 2 か所にあって片方だけが直っている」で何度も刺されている
    (補完エンジンが `win32/minibuf.cc` の写しだった件)。ここは **orphan の
    扱い** -- 走っている最中に手放したら本体が自分で片付ける -- が繊細で、
    写すと必ずずれる。

    **出したことを verify する場所が無かった。** tree-sitter のテストは
    14 件で、**非同期の経路を名指しで測っているものが 1 つも無い。**
    「ts が通ったから抽出は安全」とは言えないので、
    `src/core/worker-thread-test.cc` を足した (11 項目)。#240 で作った
    C++ テストの置き場が 2 つめの使い手を得た形である。

    **共有できる不変条件を 1 つ書いた: worker は Lisp オブジェクトを
    触らない。** `gc_mark_in_stack` が走査するスタックは 1 本なので、
    **worker だけが持っている Lisp 参照は回収される。** worker へ渡すのは
    main スレッドで snapshot した plain なメモリ (`malloc`、`xmalloc` では
    ない) に限り、結果の取り付けも main でやる。**これは機械では守れないので
    使う側が守る** -- ヘッダの先頭に書いた。

    負の確認: `pthread_create` を呼ばない形に潰す -> **11 項目のうち
    6 項目が落ちる** (exit 1)。

    **その負の確認で、テストが落ちる代わりに固まった。** 期限の無い
    `while (!flag)` を書いていたためで、**固まったテストは CI では step
    timeout として出るので「何が壊れたか」が読めない** (smoke の予算で
    同じ失敗をしたのと同じ形)。待つところ全部に期限を付けて、立たなければ
    check として落ちるようにした。

  * **端末でソケットの待ちに入ると固まって C-g で戻れなかったのを直した**
    (issue #242)。

    ```
    (connect "10.255.255.1" 80)   ; 応答しないアドレス
    変更前: 画面が固まり、C-g を押しても後続の入力が一切通らない
    変更後: C-g で quit が届き、その後も普通に動く
    ```

    `posix_wait_ready` (`src/core/sock.cc`) は 100ms の `select` を回しながら
    `Fdo_events` を呼んで `QUITP` を見ていたが、**`Fdo_events` は POSIX の
    フロントエンドで `return Qnil;` の空実装である。** `QUITP` が見るフラグを
    立てるのは入力を読む経路だけで、待っている間は誰も読まない。
    **つまりこのループは何も見ていなかった。**

    **そこに「ここで受ける」と書いてあった。** PR #227 でその関数を書いた
    ときのコメントで、100ms ごとに目を覚ますようにはなったが、目を覚まして
    呼ぶ先が空実装なので**固まる状態はそのまま残っていた。測らずに書いた。**
    issues #206 / #211 / #212 と同じ形 (スタブが成功を返すので呼ぶ側が
    「効いた」と読む) を、自分でもう一度作っていた。

    直し方は**既にあるものを呼ぶだけ**だった。`poll_quit_char`
    (`src/core/quit-poll.cc`) は走っている Lisp を C-g で止めるために
    issue #162 で入れたもので、端末を nodelay で覗いて C-g なら
    `quit-flag` を立て、**それ以外の打鍵は捨てずに入力キューの末尾へ戻す。**
    中に 50ms の下限が掛かっているので 100ms のこのループと噛み合う。
    `Frontend` に新しい仮想関数を足す必要は無かった。

    **中断は `quit` として届く** (エラーではない)。C-g の意味がそれなので
    正しい。

    測り方は**時間を測らない。**「固まらないこと」は時間の性質で、閾値を
    置くと flaky になる (PR #71 の教訓)。見るのは到達性だけで、
    **2 つ見ないと空回りする:**

      * C-g の後の eval が描かれる (= 固まっていない)
      * **中断が `quit` として届く** (= C-g が実際に届いた)。これが無いと、
        `connect` が自分で戻ってきただけでも 1 つ目は通る

    目印は**実行時に組み立てる。** `\e\e` は打った式をそのまま画面へ出すので、
    目印をソースにそのまま書くと**走っていない枝の文字列まで画面に出て
    grep が当たる** (最初にそれで通してしまった)。

    負の確認: `poll_quit_char` の呼びを消す -> この check だけが落ちる
    (「C-g で走っている Lisp が止まる」は通ったまま)。

  * **GC のスタック走査がマップされていないページを読みうる状態だったのを
    直した** (issue #240)。**POSIX で保守的 GC の guard が一度も効いて
    いなかった。**

    `gc_mark_in_stack` (`src/core/data.cc:794`) はスタックを保守的に走査して
    Lisp データ領域を指していそうな語を辿るが、その手前で
    ...
    と確かめている。**`IsBadWritePtr` は POSIX で `return FALSE;`
    (= 「悪くない」) のスタブだった**ので `continue` は一度も通らず、
    **必ずページのヘッダを読んでいた。**

    そして読めないページが実在する。`alloc_page::alloc` は 64KB を
    `MEM_RESERVE` + `PAGE_NOACCESS` で予約して **unit ごとに commit** し、
    POSIX の `VirtualAlloc` シムは予約を `PROT_NONE` で mmap する。
    実測で `---p` の領域が **commit 済みの Lisp ページに挟まれて 31 個**
    見えた。`ld_lower_bound` / `ld_upper_bound` は最小と最大を広げるだけ
    なので、そこは範囲の中である。**スタックのゴミがそこを指せば GC 中に
    SIGSEGV する。**

    **落ちるところは見ていない** (ゴミがちょうど未 commit のページを指す
    必要がある)。**ヒープが大きいほど当たりやすくなる**形なので、
    「今まで落ちていないから大丈夫」とは言えない側の問題として直した。

    直し方は**シムが commit した範囲を自分で覚える**形。予約ごとに
    `{base, size, committed}` を持ち、`committed` は OS ページ 1 枚 =
    1 ビットのビットマップ。表は base で整列した配列で二分探索する。
    **記帳に入れられなければ予約自体を失敗させる** ので、「表に無い」が
    「実は良い」を意味する余地が無い。

    **記帳はヘッダに置けない。これが設計の要点だった。** 元の
    `_va_regions` は `platform.h` の namespace スコープの `static` で、
    **翻訳単位ごとに別の実体**になる。今それで動いていたのは
    `VirtualAlloc` を呼ぶのが `alloc.cc` だけだったからで、
    **`IsBadWritePtr` を呼ぶのは `data.cc`** -- 別の翻訳単位である。
    記帳をヘッダに置いたままにすると `data.cc` から見た表は空で、
    **本物の Lisp ページ全部に「悪い」と答え、生きているオブジェクトが
    回収される。** 実体を `src/core/mem-posix.cc` へ出した
    (`vfs-posix.cc` / `dll-posix.cc` / `ini-posix.cc` と同じ並び)。

    **表に無いアドレスは「悪い」と答える。Win32 より厳しい。** 本物の
    `ldata_rep` ページは必ずこのシムを通るので偽の「悪い」は出ず、
    範囲の中に混ざるシム外の mapping (実測では tree-sitter の `.so` が
    そこに見えた) も弾ける。

    **速くなった。** ゴミのポインタが `find_object` のヒープ全走査へ進む
    のを止められるため:

    ```
    30 万 cons + GC 100 回:
      変更前 (スタブ): 2504 / 2576 / 2400 ms
      変更後 (記帳):   2241 / 2264 / 2227 ms
    ```

    **確認の置き場が無かったので作った。** 見たい性質は Lisp から呼べず、
    症状はスタックのゴミ次第なので Lisp からは作れない。**述語そのものを
    測る C++ のテストを 1 つ足した** (`src/core/mem-posix-test.cc`、19 項目、
    `tools/linux-smoke.sh` から走らせる)。**このリポジトリで最初の C++ の
    テストである** -- core とはリンクせず `mem-posix.cc` 1 つだけを持つ形に
    留めてある。

    負の確認 2 つ:

      * `IsBadWritePtr` を元の `return FALSE;` へ戻す ->
        **19 項目のうち 10 項目が落ちる** (exit 1)
      * guard が空回りしていないこと -- `---p` のページが実在することを
        smoke で数える。予約と同時に全部 commit する形へ変えたら
        この check が「もう要らない」と言う
  ...
  * **`--batch` でエラーが出たときの終了コードを 1 にした**
    (issue #236 の段取り 2)。**エラーが見えるようになっても、exit 0 の
    ままではスクリプトは気付けない。**

    ```
    $ xyzzy --batch -q -e '(car 1)'      ; echo $?   -> 1
    $ xyzzy --batch -q -e '(format t "x~%")' ; echo $?   -> 0
    $ xyzzy --batch -q -e '(warn "w")'   ; echo $?   -> 0
    ```

    **警告では立てない。** 警告は報告するが失敗ではないので、`putmsg` の
    beep の選び分けと同じ `:warning` の subtypep で分ける。

    **後から走る `kill-xyzzy` が勝つ。** `Fkill_xyzzy` は `app.exit_code` を
    引数から入れ直すので、明示的に終了コードを決めているものはそのまま
    通る -- `misc/run-tests-batch.l` が自分で pass/fail を返すのがこれで、
    ここを取り違えるとテストの結果が上書きされる。測った通り:
    ...
    *post-startup-hook* で (kill-xyzzy 5)                 -> 5
    ...
    負の確認 2 つ:
      ...
      * `app.exit_code` を立てるのをやめる -> エラーが 0 のままで落ちる
      * 警告の除外をやめる -> 警告が 1 になる方で落ちる

    **smoke の step timeout も上げた** (10 分 -> 20 分、job は 70 -> 90 分)。
    この確認を足した run が**最後の 1 件を出す手前で step timeout に当たり、
    テストが落ちたように見えた。** 原因はテストではなく予算で、pty の確認は
    1 件ごとにエディタを起動して最初の描画を待つ (cold な runner で 12 秒
    ほど) ため、**1 日で 22 件から 32 件に増やしたところで 10 分に収まらなく
    なった。** 確認を削って収めるのではなく予算を上げる方に倒し、
    ワークフローにその判断を書いた。この確認自体は 4 起動から 3 起動に
    まとめた (正常な出力と警告は 1 つの run で足りる)。
